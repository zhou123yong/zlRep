(function () {
    ...
    function init() {
        ...
        this.panel.on(typeof document.body.onmousewheel !== 'undefined' ? 'mousewheel' : 'DOMMouseScroll', function (e) {
            ...
            var elem = $(this),
                ...
                minTop;
            if (wrapper.outerHeight() <= elem.innerHeight()) {
                return;
            }
            if (delta > 0) {
                if (wrapperTop === 0) {
                ...
                minTop = elem.height() - wrapper.outerHeight(true);
    ...
    function getUsedHeight() {
        var elem = this.panel;
        return elem.innerHeight() - this.minHeight - this.padding * 2;
    }
    Scrollbar.prototype.update = function () {
        ...
        $('span', scrollbar).css({
            height: panelHeight / wrapperHeight * getUsedHeight.call(this) + ($('span', scrollbar).css('box-sizing') === 'border-box' ? this.minHeight : 0)
        });
        this.scrollTo(parseFloat(children.first().css('top')));
    };
